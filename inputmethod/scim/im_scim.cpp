...
static im_scim_context_private_t *id_to_context(int id) {
  size_t i;
...
static void cb_config_load(const ConfigPointer &config) {
  KeyEvent key;

  keymatcher_frontend.load_hotkeys(config);
  keymatcher_imengine.load_hotkeys(config);

  scim_string_to_key(key, config->read(String(SCIM_CONFIG_HOTKEYS_FRONTEND_VALID_KEY_MASK),
...
static void cb_preedit_update(IMEngineInstanceBase *instance, const WideString &wstr,
...
static void cb_preedit_caret(IMEngineInstanceBase *instance, int caret) {
  ...
  if (context && context->on) {
    ...
    (*context->cb->preedit_update)(context->self, C_STR(context->preedit_str), caret);
  }
}
...
static void cb_prop_register(IMEngineInstanceBase *instance, const PropertyList &props) {
  im_scim_context_private_t *context;
...
static void cb_prop_update(IMEngineInstanceBase *instance, const Property &prop) {
  im_scim_context_private_t *context;
...
static void set_callbacks(im_scim_context_private_t *context) {
...
static void cb_panel_request_factory_menu(int id) {
  ...
  for (i = 0; i < factories.size(); i++) {
    menu.push_back(PanelFactoryInfo(factories[i]->get_uuid(),
                                    ...
                                    factories[i]->get_language(), factories[i]->get_icon_file()));
  }
...
static void cb_panel_request_help(int id) {
  im_scim_context_private_t *context;
  String desc;
  String str;
  ...
  desc += utf8_wcstombs(context->factory->get_name()) + String(":\n\n");
...
static void cb_panel_change_factory(int id, const String &uuid) {
  im_scim_context_private_t *context;
  IMEngineFactoryPointer factory;
  ...
  info = PanelFactoryInfo(context->factory->get_uuid(), utf8_wcstombs(context->factory->get_name()),
                          context->factory->get_language(), context->factory->get_icon_file());
  panel_client.update_factory_info(id, info);
  panel_client.send();
  context->instance->focus_in();
}

static void cb_panel_trigger_property(int id, const String &prop) {
  im_scim_context_private_t *context;
...
static int hotkey(im_scim_context_t _context, const KeyEvent &scim_key) {
  im_scim_context_private_t *context;
  ...
  switch (hotkey_action) {
    case SCIM_FRONTEND_HOTKEY_ON:
      ...
      (*context->cb->preedit_update)(context->self, C_STR(context->preedit_str),
...
int im_scim_initialize(char *locale) {
  ...
  if (!client.connect(address) &&
      !scim_socket_open_connection(magic, String("ConnectionTester"), String("SocketFrontEnd"),
                                   client, TIMEOUT)) {
    bl_error_printf("Unable to connect to the socket frontend.\n");
    goto error;
  }

  if (scim_get_imengine_module_list(imengines) == 0) {
    bl_error_printf("Could not find any IMEngines.\n");
    goto error;
  }

  if (std::find(imengines.begin(), imengines.end(), "socket") == imengines.end()) {
    bl_error_printf("Could not find socket module.\n");
    goto error;
  }
  ...
  if (scim_get_config_module_list(config_modules) == 0) {
    bl_error_printf("Could not find any config modules.\n");
    goto error;
  }

  config_mod_name = scim_global_config_read(SCIM_GLOBAL_CONFIG_DEFAULT_CONFIG_MODULE,
                                            String("simple"));  // String( "socket"));

  if (std::find(config_modules.begin(), config_modules.end(), config_mod_name) ==
      config_modules.end()) {
    // fallback
  ...
  if (!(config_module = new ConfigModule(config_mod_name))) {
    bl_error_printf("ConfigModule failed. (%s)\n", config_mod_name.c_str());
    goto error;
  }
  ...
  if (config.null()) {
    ...
    bl_error_printf("create_config failed.\n");
    goto error;
  }

  be = new CommonBackEnd(config, imengines);
  if (be.null()) {
    bl_error_printf("CommonBackEnd failed.\n");
    goto error;
  }
...
im_scim_context_t im_scim_create_context(void *self, im_scim_callbacks_t *callbacks) {
  ...
  if (!(context->instance = context->factory->create_instance(String("UTF-8"), next_new_id))) {
    bl_error_printf("Could not create new instance.\n");
    return NULL;
  }
...
int im_scim_destroy_context(im_scim_context_t _context) {
  im_scim_context_private_t *context;
  ...
  context_table.erase(std::find(context_table.begin(), context_table.end(), context));
...
int im_scim_focused(im_scim_context_t _context) {
  im_scim_context_private_t *context;
...
int im_scim_unfocused(im_scim_context_t _context) {
  im_scim_context_private_t *context;
...
unsigned int im_scim_preedit_char_attr(im_scim_context_t _context, unsigned int index) {
  im_scim_context_private_t *context;
  ...
  for (attr = context->preedit_attr.begin(); attr != context->preedit_attr.end(); attr++) {
    unsigned int start;
    unsigned int end;
