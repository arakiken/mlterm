	...
	)
{
	...
	else
	{
		...
		if( ( p = realloc( canna->im.preedit.chars ,
		...
		(*syms->ml_str_init)( canna->im.preedit.chars = p ,
		...
		while( (*canna->parser_term->next_char)( canna->parser_term , &ch))
		{
			...
			if( (*syms->ml_convert_to_internal_ch)( &ch ,
				(*canna->im.listener->get_unicode_policy)(
					canna->im.listener->self) ,
				US_ASCII) <= 0)
			{
				continue ;
			}
	...
	(*canna->im.listener->draw_preedit_str)( canna->im.listener->self ,
					       canna->im.preedit.chars ,
					       canna->im.preedit.filled_len ,
...
candidate:
	...
	else
	{
		...
		(*canna->im.listener->get_spot)( canna->im.listener->self ,
					       canna->im.preedit.chars ,
					       canna->im.preedit.segment_offset ,
					       &x , &y) ;

		if( canna->im.stat_screen == NULL)
		{
			if( ! ( canna->im.stat_screen = (*syms->x_im_status_screen_new)(
					canna->im.disp , canna->im.font_man ,
					canna->im.color_man ,
					(*canna->im.listener->is_vertical)(
						canna->im.listener->self) ,
					(*canna->im.listener->get_line_height)(
						canna->im.listener->self) ,
					x , y)))
			{
			#ifdef  DEBUG
		...
		(*canna->im.stat_screen->set)( canna->im.stat_screen ,
			canna->parser_term , candidateword) ;
	...
	)
{
	...
	if( ! NEED_TO_CONV(canna))
	{
		(*canna->im.listener->write_to_term)( canna->im.listener->self ,
						    (u_char*)str , len) ;
	...
	{
		filled_len = (*canna->conv->convert)( canna->conv , conv_buf ,
						    sizeof( conv_buf) ,
						    parser_eucjp) ;
	...
	)
{
	...
	else if( canna->is_enabled)
	{
		...
		len = jrKanjiString( 0 , ksym ,
				canna->buf , sizeof(canna->buf) , &canna->key_status) ;
		...
		if( ( canna->key_status.info & KanjiGLineInfo) &&
		    canna->key_status.gline.length > 0)
		{
			u_char *  p ;
		...
		{
			preedit( canna ,
				canna->key_status.length > 0 ? canna->key_status.echoStr : "" ,
				canna->key_status.revPos ,
				canna->key_status.revLen , cand) ;
		}
...
{
	return  ((im_canna_t*)im)->is_enabled ;
}
	...
	)
{
	...
	if( ref_count == 0)
	{
		...
		if( ! ( parser_eucjp = (*syms->ml_parser_new)( ML_EUCJP)))
		{
			return  NULL ;
		}
	}
	...
	{
		if( ! ( canna->conv = (*syms->ml_conv_new)( term_encoding)))
		{
			goto  error ;
		}
	}

	if( ! ( canna->parser_term = (*syms->ml_parser_new)( term_encoding)))
	{
		goto  error ;
	}
