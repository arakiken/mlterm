...
public class MLTermPty {
  ...
  private static void loadLibraryFromJar() {
      ...
      while (urls.hasMoreElements()) {
        ...
        if (beg_pos != -1) {
          ...
          if (end_pos != -1 && /* is jar file. */
              ...
              url.getPath().lastIndexOf(System.getProperty("file.separator"), end_pos) < beg_pos) {
    ...
    for (int count = 0; count < files.length; count++) {
      ...
      InputStream is =
          Thread.currentThread().getContextClassLoader().getResourceAsStream(files[count]);
      try {
  ...
  private native long nativeOpen(
      String host, String pass, int cols, int rows, String encoding, String[] argv);
  public boolean open(
      String host, String pass, int cols, int rows, String encoding, String[] argv) {
  ...
  public boolean isTrackingMouse(int button, boolean isMotion) {
    return nativeIsTrackingMouse(nativeObj, button, isMotion);
  }

  private native void nativeReportMouseTracking(long obj, int char_index, int row, int button,
      int state, boolean isMotion, boolean isReleased);
  public void reportMouseTracking(
      int char_index, int row, int button, int state, boolean isMotion, boolean isReleased) {
    nativeReportMouseTracking(nativeObj, char_index, row, button, state, isMotion, isReleased);
  }
