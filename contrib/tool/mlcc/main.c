...
static void help_msg(void) {
  ...
  printf(
      " mlcc exec [command]: Execute mlterm command. (full_reset, mlclient, "
      "open_pty and so on)\n");
  printf(" mlcc [key]         : Get current value of [key].\n");
  printf(" mlcc [key] [value] : Set [value] for [key].\n");
  printf(
      " mlcc [font file name] [charset] : Get font name of [charset] in [font "
      "file name].\n");
  printf(
      " mlcc [font file name] [charset] [font name] : Set [font name] in [font "
      "file name].\n");
  printf(" mlcc color [color name] [rgb] : Set [rgb] for [color name].\n");
  printf(" mlcc color [color name] : Get rgb of [color name].\n");
  printf(
      " (See doc/en/PROTOCOL, PROTOCOL.font and PROTOCOL.color for "
...
static void signal_int(int signo) { finalize(); }
...
section_t *current_section(config_data_t *data) { return &(data->section[data->selected]); }

section_t *section_from_name(config_data_t *data, char *name) {
  int i;

  for (i = 0; i < data->filled; i++)
    if (strcmp(data->section[i].name, name) == 0) return &(data->section[i]);
  return NULL;
}

section_t *section_add(config_data_t *data, const char *name) {
  if (data->filled > MAX_SECTION - 2) return NULL;
...
int display_section(window_t *window, config_data_t *data) {
  ...
  while (cur < data->filled) {
    ...
    if ((pos + len) > (window->right - window->left)) {
      ...
      display_str(window, pos + 1, skip + 1, (data->section[cur].name),
                  (data->selected == cur) ? (data->state == DS_SELECT ? DC_POINTED : DC_CHOOSED)
                                          : DC_NORMAL);
      pos += len;
    }
    cur++;
...
int data_reset(config_data_t *data) {
  ...
  for (i = 0; i < data->filled; i++) section_reset(&(data->section[i]));
  return 0;
}

void data_free(config_data_t *data) {
  for (; data->filled > 0; (data->filled)--) section_free(&data->section[data->filled - 1]);
}
...
int edit_entry(window_t *window, config_data_t *data) {
  ...
  if (result < 0) data->state = DS_SELECT;

  entry_apply(entry);
  if (entry->modified) section->modified = 1;
  return 1;
}
...
int display_entry(window_t *window, config_data_t *data) {
  ...
  for (i = 0; i < current->size; i++) {
    display_str(window, 1, i * 2, (current->entry[i].name),
                (current->selected == i) ? (data->state == DS_SELECT ? DC_CHOOSED : DC_POINTED)
                                         : DC_NORMAL);

    window_addstr(window, current->maxwidth + 3, i * 2, ":");
    if ((i == current->selected) && (data->state == DS_EDIT))
      entry_display(window, &(current->entry[i]), current->maxwidth + 4, i * 2, 1);
    else
      entry_display(window, &(current->entry[i]), current->maxwidth + 4, i * 2, 0);
  }
  return 0;
}
...
int select_entry(config_data_t *data) {
  ...
  switch (buffer) {
    ...
    case KEY_UP:
      ...
      return 1;    /* redraw */
    case KEY_DOWN: /* DOWN */
      if (cur->selected < cur->size - 1) cur->selected++;
      return 1;     /* redraw */
    case KEY_RIGHT: /* RIGHT */
      if (data->selected < data->filled - 1) data->selected++;
      return 1;    /* redraw */
    case KEY_LEFT: /* LEFT */
...
int main(int argc, char **argv) {
  ...
  if (argc == 2) {
    ...
    while (++i < argc) {
      int enclose;
      strcat(cmd, " ");
      if (strncmp(argv[2], "mlclient", 8) == 0 || strcmp(argv[2], "snapshot") == 0 ||
          strcmp(argv[2], "show_picture") == 0 || strcmp(argv[2], "scp") == 0) {
        enclose = 1;
        strcat(cmd, "\"");
      } else {
        enclose = 0;
  ...
  if (!getenv("MLTERM")) {
    ...
    printf(
        "mlcc without options doesn't work on a terminal except mlterm "
        "or on a remote host.\n");
    exit(1);
  }
  ...
  init_data(&data);
  if (term_size(&cols, &rows) == -1) goto FIN;
...
CANCEL:
  ...
  data_reset(&data);
FIN:
  data_free(&data); /* destruct date tree. segv should be caught here if there
                       are bugs*/
