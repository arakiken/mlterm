...
typedef struct {
  ...
  /* Actual width, while ui_display_t.width excludes virtual kbd area. */
  unsigned int width;
  /* Actual height, while ui_display_t.height excludes virtual kbd area. */
  unsigned int height;
...
/* Same as fb/ui.h (to use same inputmethod plugins from both mlterm-fb and
 * mlterm-con) */
...
#else /* if __FreeBSD__ || __linux__ || __darwin__ */

#ifndef __linux__ /* FreeBSD / MacOSX etc */
