...
#include "../src/ef_ucs4_iscii.h"

#define ISCIICS_TO_INDEX(cs) ((cs) == ISCII_ASSAMESE ? 0 : (cs)-ISCII_BENGALI)
...
static u_int8_t convert_ucs_to_iscii(u_int16_t ucs, ef_charset_t cs) {
  u_int8_t iscii;
...
static int convert_iscii_to_ucs4(ef_char_t* ucs4, u_int16_t iscii_code, ef_charset_t cs) {
  if (iscii_code <= 0x9f) {
    ...
    ef_int_to_bytes(ucs4->ch, 4, ucs_code);
    ucs4->size = 4;
    ucs4->cs = ISO10646_UCS4_1;
    ucs4->property = 0;
