...
static int
check_font_config(
	const char *  default_family	/* family name */
	)
{
	const char *  path ;
		...
		for( m_idx = 0 ; ; m_idx++)
		{
			...
			if( m_idx == num_of_matches)
			{
				...
				if( ! ( matches[num_of_matches] = search_next_font( pattern)))
				{
					...
					FcObjectSet * objset = FcObjectSetBuild( FC_FAMILY ,
								FC_FILE , FC_CHARSET , NULL) ;
					...
					for( count = 0 ; count < fontset->nfont ; count++)
					{
						...
						FcPatternGet( matches[num_of_matches - 1] ,
							FC_FAMILY , 0 , &val) ;
					#if  0
			...
			if( FcPatternGetCharSet( matches[m_idx] , FC_CHARSET , 0 , &charset)
				== FcResultMatch &&
			    ( FcCharSetHasChar( charset , blocks[b_idx].beg) ||
			      FcCharSetHasChar( charset , blocks[b_idx].beg + 1)))
			{
				if( FcPatternGet( matches[m_idx] , FC_FAMILY , 0 , &val)
					== FcResultMatch &&
				    strcmp( val.u.s , default_family) != 0)
				{
					if( num_of_outputs > 0 &&
					    outputs[num_of_outputs - 1].block->end + 1 ==
						blocks[b_idx].beg &&
					    strcmp( outputs[num_of_outputs - 1].family ,
						val.u.s) == 0)
					{
						/* XXX Change blocks[b_idx - 1].end */
						outputs[num_of_outputs - 1].block->end =
							blocks[b_idx].end ;
					}
					else
					{
						...
						if( FcPatternGet( matches[m_idx] , FC_FILE ,
							0 , &val) == FcResultMatch)
						{
	...
	if( ( path = get_user_rc_path( "mlterm/aafont")))
	{
		...
		{
			fprintf( fp , "U+%x-%x=%s\n" ,
				outputs[count].block->beg ,
				outputs[count].block->end ,
