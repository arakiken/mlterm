...
static char *config_keys[MC_FLAG_MODES] = {
    ...
    "static_backscroll_mode", "use_extended_scroll_shortcut", "regard_uri_as_word", "use_ot_layout",
};
...
GtkWidget *mc_flag_config_widget_new(int id) {
  if (id == MC_FLAG_XFT) {
    ...
    old_flag_modes[id] = new_flag_modes[id] = (strcmp(mc_get_str_value(config_keys[id]), "2") == 0);
  } else {
...
void mc_update_flag_mode(int id) {
  ...
  if (is_changed[id]) {
    if (id == MC_FLAG_XFT) {
      ...
      } else if (!gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets[MC_FLAG_CAIRO]))) {
        mc_set_str_value(config_keys[id], "xcore");
      }
    } else if (id == MC_FLAG_CAIRO) {
      if (new_flag_modes[id]) {
        mc_set_str_value(config_keys[id], "cairo");
      } else if (!gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets[MC_FLAG_XFT]))) {
        mc_set_str_value(config_keys[id], "xcore");
      }
    } else if (id == MC_FLAG_AWIDTH) {
      if (new_flag_modes[id]) {
