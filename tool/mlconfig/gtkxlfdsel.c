...
/*
 * Modified by Araki Ken(araklen@users.sf.net).
...
#ifndef G_PLATFORM_WIN32 /* In case of compiling with cygwin + X11 + win32 \
                            native gtk+. */
...
#include <gtk/gtkversion.h> /* include <gtk/gtkfeatures.h> in GTK 1.2 */
#else
#include <gtk/gtk.h>
#endif
...
static const guint16 font_sizes[] = {8,  9,  10, 11, 12, 13, 14, 16, 18, 20, 22,
...
#define INITIAL_METRIC GTK_XLFD_METRIC_POINTS
...
typedef struct _GtkXlfdSelInfo GtkXlfdSelInfo;
...
/* This represents one style, as displayed in the Font Style clist. It can
   ...
   GtkXlfdType in the flags field. */
#define GTK_XLFD_DISPLAYED (1 << 7)
struct _FontStyle {
...
static const gchar *xlfd_field_names[GTK_XLFD_NUM_FIELDS] = {
    N_("Foundry:"),      N_("Family:"),       N_("Weight:"),     N_("Slant:"),
    N_("Set Width:"),    N_("Add Style:"),    N_("Pixel Size:"), N_("Point Size:"),
    N_("Resolution X:"), N_("Resolution Y:"), N_("Spacing:"),    N_("Average Width:"),
    N_("Charset:"),
};
...
static const FontField xlfd_index[GTK_NUM_FONT_PROPERTIES] = {
    XLFD_WEIGHT, XLFD_SLANT, XLFD_SET_WIDTH, XLFD_SPACING, XLFD_CHARSET, XLFD_FOUNDRY};
...
static const gint filter_heights[GTK_NUM_FONT_PROPERTIES] = {100, 70, 70, 40, 100, 100};

/* This is returned by gtk_xlfd_selection_filter_state to describe if a
...
typedef enum { FILTERED, NOT_FILTERED, NOT_SET } GtkXlfdPropertyFilterState;

static GtkXlfdSelInfo *fontsel_info = NULL;

/* The initial size and increment of each of the arrays of property values. */
#define PROPERTY_ARRAY_INCREMENT 16

static void gtk_xlfd_selection_class_init(GtkXlfdSelectionClass *klass);
static void gtk_xlfd_selection_init(GtkXlfdSelection *fontsel);
static void gtk_xlfd_selection_destroy(GtkObject *object);

/* These are all used for class initialization - loading in the fonts etc. */
static void gtk_xlfd_selection_get_fonts(void);
static void gtk_xlfd_selection_insert_font(GSList *fontnames[], gint *ntable, gchar *fontname);
static gint gtk_xlfd_selection_insert_field(gchar *fontname, gint prop);

/* These are the callbacks & related functions. */
static void gtk_xlfd_selection_select_font(GtkWidget *w, gint row, gint column,
                                           GdkEventButton *bevent, gpointer data);
static gint gtk_xlfd_selection_on_clist_key_press(GtkWidget *clist, GdkEventKey *event,
                                                  GtkXlfdSelection *fs);
static gboolean gtk_xlfd_selection_select_next(GtkXlfdSelection *fs, GtkWidget *clist, gint step);
static void gtk_xlfd_selection_show_available_styles(GtkXlfdSelection *fs);
static void gtk_xlfd_selection_select_best_style(GtkXlfdSelection *fs, gboolean use_first);

static void gtk_xlfd_selection_select_style(GtkWidget *w, gint row, gint column,
                                            GdkEventButton *bevent, gpointer data);
static void gtk_xlfd_selection_show_available_sizes(GtkXlfdSelection *fs);
static void gtk_xlfd_selection_size_activate(GtkWidget *w, gpointer data);

static void gtk_xlfd_selection_select_best_size(GtkXlfdSelection *fs);
static void gtk_xlfd_selection_select_size(GtkWidget *w, gint row, gint column,
                                           GdkEventButton *bevent, gpointer data);

static void gtk_xlfd_selection_metric_callback(GtkWidget *w, gpointer data);
static void gtk_xlfd_selection_expose_list(GtkWidget *w, GdkEventExpose *event, gpointer data);
static void gtk_xlfd_selection_realize_list(GtkWidget *widget, gpointer data);

static void gtk_xlfd_selection_switch_page(GtkWidget *w, GtkNotebookPage *page, gint page_num,
                                           gpointer data);
static void gtk_xlfd_selection_show_font_info(GtkXlfdSelection *fs);

static void gtk_xlfd_selection_select_filter(GtkWidget *w, gint row, gint column,
                                             GdkEventButton *bevent, GtkXlfdSelection *fs);
static void gtk_xlfd_selection_update_filter(GtkXlfdSelection *fs);
static gboolean gtk_xlfd_selection_style_visible(GtkXlfdSelection *fs, FontInfo *font, gint style);
static void gtk_xlfd_selection_reset_filter(GtkWidget *w, GtkXlfdSelection *fs);
static void gtk_xlfd_selection_on_clear_filter(GtkWidget *w, GtkXlfdSelection *fs);
static void gtk_xlfd_selection_show_available_fonts(GtkXlfdSelection *fs);
static void gtk_xlfd_selection_clear_filter(GtkXlfdSelection *fs);
static void gtk_xlfd_selection_update_filter_lists(GtkXlfdSelection *fs);
static GtkXlfdPropertyFilterState gtk_xlfd_selection_filter_state(GtkXlfdSelection *fs,
                                                                  GtkXlfdFilterType filter_type,
                                                                  gint property, gint index);

/* Misc. utility functions. */
static gboolean gtk_xlfd_selection_load_font(GtkXlfdSelection *fs);
static void gtk_xlfd_selection_update_preview(GtkXlfdSelection *fs);

static gint gtk_xlfd_selection_find_font(GtkXlfdSelection *fs, gchar *family, guint16 foundry);
static guint16 gtk_xlfd_selection_field_to_index(gchar **table, gint ntable, gchar *field);

static gchar *gtk_xlfd_selection_expand_slant_code(gchar *slant);
static gchar *gtk_xlfd_selection_expand_spacing_code(gchar *spacing);

/* Functions for handling X Logical Font Description fontnames. */
static gboolean gtk_xlfd_selection_is_xlfd_font_name(const gchar *fontname);
static char *gtk_xlfd_selection_get_xlfd_field(const gchar *fontname, FontField field_num,
                                               gchar *buffer);
static gchar *gtk_xlfd_selection_create_xlfd(gint size, GtkXlfdMetricType metric, gchar *foundry,
                                             gchar *family, gchar *weight, gchar *slant,
                                             gchar *set_width, gchar *spacing, gchar *charset);

/* FontSelectionDialog */
static void gtk_xlfd_selection_dialog_class_init(GtkXlfdSelectionDialogClass *klass);
static void gtk_xlfd_selection_dialog_init(GtkXlfdSelectionDialog *fontseldiag);

static gint gtk_xlfd_selection_dialog_on_configure(GtkWidget *widget, GdkEventConfigure *event,
                                                   GtkXlfdSelectionDialog *fsd);

static GtkWindowClass *font_selection_parent_class = NULL;
static GtkNotebookClass *font_selection_dialog_parent_class = NULL;

GtkType gtk_xlfd_selection_get_type() {
  static GtkType font_selection_type = 0;

  if (!font_selection_type) {
    static const GtkTypeInfo fontsel_type_info = {
        "GtkXlfdSelection", sizeof(GtkXlfdSelection), sizeof(GtkXlfdSelectionClass),
        (GtkClassInitFunc)gtk_xlfd_selection_class_init, (GtkObjectInitFunc)gtk_xlfd_selection_init,
        /* reserved_1 */ NULL,
        /* reserved_2 */ NULL, (GtkClassInitFunc)NULL,
    };

    font_selection_type = gtk_type_unique(GTK_TYPE_NOTEBOOK, &fontsel_type_info);
  }
...
static void gtk_xlfd_selection_class_init(GtkXlfdSelectionClass *klass) {
  ...
  object_class->destroy = gtk_xlfd_selection_destroy;

  gtk_xlfd_selection_get_fonts();
}
...
static void gtk_xlfd_selection_init(GtkXlfdSelection *fontsel) {
  ...
  /* Initialize the GtkXlfdSelection struct. We do this here in case any
  ...
  fontsel->filters[GTK_XLFD_FILTER_BASE].font_type = GTK_XLFD_ALL;
  fontsel->filters[GTK_XLFD_FILTER_USER].font_type = GTK_XLFD_BITMAP | GTK_XLFD_SCALABLE;

  for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++) {
    fontsel->filters[GTK_XLFD_FILTER_BASE].property_filters[prop] = NULL;
    fontsel->filters[GTK_XLFD_FILTER_BASE].property_nfilters[prop] = 0;
    fontsel->filters[GTK_XLFD_FILTER_USER].property_filters[prop] = NULL;
    fontsel->filters[GTK_XLFD_FILTER_USER].property_nfilters[prop] = 0;
  }

  for (prop = 0; prop < GTK_NUM_STYLE_PROPERTIES; prop++) fontsel->property_values[prop] = 0;
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(fontsel), fontsel->main_vbox, label);
  ...
  gtk_table_attach(GTK_TABLE(table), fontsel->font_label, 0, 1, 0, 1, GTK_FILL, 0, 0, 0);
  ...
  gtk_table_attach(GTK_TABLE(table), fontsel->font_entry, 0, 1, 1, 2, GTK_FILL, 0, 0, 0);
  ...
  gtk_table_attach(GTK_TABLE(table), fontsel->font_style_entry, 1, 2, 1, 2, GTK_FILL, 0, 0, 0);
  fontsel->size_entry = gtk_entry_new();
  gtk_widget_set_usize(fontsel->size_entry, 20, -1);
  gtk_widget_show(fontsel->size_entry);
  gtk_table_attach(GTK_TABLE(table), fontsel->size_entry, 2, 3, 1, 2, GTK_FILL, 0, 0, 0);
  ...
  gtk_table_attach(GTK_TABLE(table), scrolled_win, 0, 1, 2, 3, GTK_EXPAND | GTK_FILL,
                   GTK_EXPAND | GTK_FILL, 0, 0);

  fontsel->font_style_clist = gtk_clist_new(1);
  gtk_clist_column_titles_hide(GTK_CLIST(fontsel->font_style_clist));
  gtk_clist_set_column_auto_resize(GTK_CLIST(fontsel->font_style_clist), 0, TRUE);
  scrolled_win = gtk_scrolled_window_new(NULL, NULL);
  ...
  gtk_widget_show(scrolled_win);
  gtk_table_attach(GTK_TABLE(table), scrolled_win, 1, 2, 2, 3, GTK_EXPAND | GTK_FILL,
                   GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_widget_show(scrolled_win);
  gtk_table_attach(GTK_TABLE(table), scrolled_win, 2, 3, 2, 3, GTK_FILL, GTK_FILL, 0, 0);
  ...
  gtk_xlfd_selection_show_available_fonts(fontsel);

  gtk_signal_connect(GTK_OBJECT(fontsel->font_clist), "select_row",
                     GTK_SIGNAL_FUNC(gtk_xlfd_selection_select_font), fontsel);
  GTK_WIDGET_SET_FLAGS(fontsel->font_clist, GTK_CAN_FOCUS);
  gtk_signal_connect(GTK_OBJECT(fontsel->font_clist), "key_press_event",
                     GTK_SIGNAL_FUNC(gtk_xlfd_selection_on_clist_key_press), fontsel);
  gtk_signal_connect_after(GTK_OBJECT(fontsel->font_clist), "map_event",
                           GTK_SIGNAL_FUNC(gtk_xlfd_selection_expose_list), fontsel);

  gtk_signal_connect(GTK_OBJECT(fontsel->font_style_clist), "select_row",
                     GTK_SIGNAL_FUNC(gtk_xlfd_selection_select_style), fontsel);
  GTK_WIDGET_SET_FLAGS(fontsel->font_style_clist, GTK_CAN_FOCUS);
  gtk_signal_connect(GTK_OBJECT(fontsel->font_style_clist), "key_press_event",
                     GTK_SIGNAL_FUNC(gtk_xlfd_selection_on_clist_key_press), fontsel);
  gtk_signal_connect_after(GTK_OBJECT(fontsel->font_style_clist), "realize",
                           GTK_SIGNAL_FUNC(gtk_xlfd_selection_realize_list), fontsel);

  /* Insert the standard font sizes */
  gtk_clist_freeze(GTK_CLIST(fontsel->size_clist));
  size_to_match = INITIAL_FONT_SIZE;
  if (INITIAL_METRIC == GTK_XLFD_METRIC_POINTS) size_to_match = size_to_match / 10;
  ...
  gtk_signal_connect(GTK_OBJECT(fontsel->size_clist), "select_row",
                     GTK_SIGNAL_FUNC(gtk_xlfd_selection_select_size), fontsel);
  ...
  gtk_misc_set_padding(GTK_MISC(GTK_BIN(fontsel->filter_button)->child), 16, 0);
  ...
  gtk_signal_connect(GTK_OBJECT(fontsel->filter_button), "clicked",
                     GTK_SIGNAL_FUNC(gtk_xlfd_selection_on_clear_filter), fontsel);
  ...
  if (INITIAL_METRIC == GTK_XLFD_METRIC_POINTS)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->points_button), TRUE);

  fontsel->pixels_button = gtk_radio_button_new_with_label_from_widget(
      GTK_RADIO_BUTTON(fontsel->points_button), _("Pixels"));
  gtk_widget_show(fontsel->pixels_button);
  gtk_box_pack_start(GTK_BOX(hbox2), fontsel->pixels_button, FALSE, TRUE, 0);
  if (INITIAL_METRIC == GTK_XLFD_METRIC_PIXELS)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->pixels_button), TRUE);

  gtk_signal_connect(GTK_OBJECT(fontsel->points_button), "toggled",
                     (GtkSignalFunc)gtk_xlfd_selection_metric_callback, fontsel);
  ...
  gtk_box_pack_start(GTK_BOX(fontsel->main_vbox), text_frame, FALSE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(text_box), fontsel->preview_entry, TRUE, TRUE, 0);

  /* Create the message area */
  fontsel->message_label = gtk_label_new("");
  gtk_widget_show(fontsel->message_label);
  gtk_box_pack_start(GTK_BOX(fontsel->main_vbox), fontsel->message_label, FALSE, FALSE, 0);
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(fontsel), fontsel->info_vbox, label);
  ...
  gtk_widget_show(scrolled_win);
  gtk_box_pack_start(GTK_BOX(fontsel->info_vbox), scrolled_win, TRUE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(fontsel->info_vbox), fontsel->requested_font_name, FALSE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(fontsel->info_vbox), fontsel->actual_font_name, FALSE, TRUE, 0);

  sprintf(buffer, _("%i fonts available with a total of %i styles."), fontsel_info->nfonts,
  ...
  gtk_signal_connect(GTK_OBJECT(fontsel), "switch_page",
                     GTK_SIGNAL_FUNC(gtk_xlfd_selection_switch_page), fontsel);
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(fontsel), fontsel->filter_vbox, label);
  ...
  gtk_box_pack_start(GTK_BOX(hbox2), fontsel->type_bitmaps_button, FALSE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(hbox2), fontsel->type_scalable_button, FALSE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(hbox2), fontsel->type_scaled_bitmaps_button, FALSE, TRUE, 0);
  ...
  for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++) {
    ...
    gtk_widget_show(label);
    gtk_table_attach(GTK_TABLE(table), label, left, left + 1, top, top + 1, GTK_FILL, GTK_FILL, 0,
                     0);
    ...
    if (top == 2 && left == 2) {
      ...
      gtk_table_attach(GTK_TABLE(table), vbox, left, left + 1, top + 1, top + 2, GTK_FILL, GTK_FILL,
                       0, 0);
      ...
      gtk_signal_connect(GTK_OBJECT(button), "clicked",
                         GTK_SIGNAL_FUNC(gtk_xlfd_selection_reset_filter), fontsel);
    } else
      gtk_table_attach(GTK_TABLE(table), scrolled_win, left, left + 1, top + 1, top + 2,
                       GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);

    gtk_signal_connect(GTK_OBJECT(clist), "select_row",
                       GTK_SIGNAL_FUNC(gtk_xlfd_selection_select_filter), fontsel);
    ...
    for (i = 1; i < fontsel_info->nproperties[prop]; i++) {
      ...
      if (prop == SLANT)
        property = gtk_xlfd_selection_expand_slant_code(property);
      else if (prop == SPACING)
        property = gtk_xlfd_selection_expand_spacing_code(property);
      ...
      if (!inserted) row = gtk_clist_append(GTK_CLIST(clist), &property);
...
GtkWidget *gtk_xlfd_selection_new() {
  GtkXlfdSelection *fontsel;

  fontsel = gtk_type_new(GTK_TYPE_XLFD_SELECTION);
...
static void gtk_xlfd_selection_destroy(GtkObject *object) {
  ...
  g_return_if_fail(GTK_IS_XLFD_SELECTION(object));

  fontsel = GTK_XLFD_SELECTION(object);

#if 0
  /* All we have to do is unref the font, if we have one. */
  if (fontsel->font) gdk_font_unref(fontsel->font);
#endif
...
/* This is called when the clist is exposed. Here we scroll to the current
   font if necessary. */
static void gtk_xlfd_selection_expose_list(GtkWidget *widget, GdkEventExpose *event,
                                           gpointer data) {
  GtkXlfdSelection *fontsel;
  FontInfo *font_info;
  GList *selection;
...
#endif
  fontsel = GTK_XLFD_SELECTION(data);
  ...
  if (selection) {
    ...
    if (gtk_clist_row_is_visible(GTK_CLIST(fontsel->font_clist), index) != GTK_VISIBILITY_FULL)
  ...
  if (selection) {
    ...
    if (gtk_clist_row_is_visible(GTK_CLIST(fontsel->font_style_clist), index) !=
        GTK_VISIBILITY_FULL)
      gtk_clist_moveto(GTK_CLIST(fontsel->font_style_clist), index, -1, 0.5, 0);
  }
  ...
  if (selection) {
    index = GPOINTER_TO_INT(selection->data);
    if (gtk_clist_row_is_visible(GTK_CLIST(fontsel->size_clist), index) != GTK_VISIBILITY_FULL)
...
static void gtk_xlfd_selection_realize_list(GtkWidget *widget, gpointer data) {
  ...
  for (row = 0; row < GTK_CLIST(fontsel->font_style_clist)->rows; row++) {
    if (GPOINTER_TO_INT(gtk_clist_get_row_data(GTK_CLIST(fontsel->font_style_clist), row)) == -1) {
      gtk_clist_set_foreground(GTK_CLIST(fontsel->font_style_clist), row, inactive_fg);
      gtk_clist_set_background(GTK_CLIST(fontsel->font_style_clist), row, inactive_bg);
    }
  }
}
...
static void gtk_xlfd_selection_select_font(GtkWidget *w, gint row, gint column,
  ...
  if (bevent && !GTK_WIDGET_HAS_FOCUS(w)) gtk_widget_grab_focus(w);
  ...
  gtk_xlfd_selection_show_available_styles(fontsel);
  gtk_xlfd_selection_select_best_style(fontsel, TRUE);
}
...
static gint gtk_xlfd_selection_on_clist_key_press(GtkWidget *clist, GdkEventKey *event,
  ...
  if (event->keyval == GDK_Up)
    return gtk_xlfd_selection_select_next(fontsel, clist, -1);
...
static gboolean gtk_xlfd_selection_select_next(GtkXlfdSelection *fontsel, GtkWidget *clist,
                                               gint step) {
  GList *selection;
  ...
  for (row = current_row + step; row >= 0 && row < GTK_CLIST(clist)->rows; row += step) {
    /* If this is the style clist, make sure that the item is not a charset
       entry. */
    if (clist == fontsel->font_style_clist)
      if (GPOINTER_TO_INT(gtk_clist_get_row_data(GTK_CLIST(clist), row)) == -1) continue;
...
static void gtk_xlfd_selection_show_available_styles(GtkXlfdSelection *fontsel) {
  FontInfo *font;
  FontStyle *styles;
  ...
  for (style = 0; style < font->nstyles; style++) {
    if (gtk_xlfd_selection_style_visible(fontsel, font, style)) {
      styles[style].flags &= ~GTK_XLFD_DISPLAYED;
  ...
  for (style = 0; style < font->nstyles; style++) {
    if (styles[style].flags & GTK_XLFD_DISPLAYED) continue;

    if (show_charset) {
      charset_index = styles[style].properties[CHARSET];
      charset = fontsel_info->properties[CHARSET][charset_index];
      row = gtk_clist_append(GTK_CLIST(fontsel->font_style_clist), &charset);
      gtk_clist_set_row_data(GTK_CLIST(fontsel->font_style_clist), row, (gpointer)-1);
    ...
    for (tmpstyle = style; tmpstyle < font->nstyles; tmpstyle++) {
      if (styles[tmpstyle].flags & GTK_XLFD_DISPLAYED ||
          charset_index != styles[tmpstyle].properties[CHARSET])
        continue;

      styles[tmpstyle].flags |= GTK_XLFD_DISPLAYED;
      ...
      if (!g_strcasecmp(slant, "r"))
        slant = NULL;
      else if (!g_strcasecmp(slant, "(nil)"))
        slant = NULL;
      else if (!g_strcasecmp(slant, "i"))
        slant = N_("italic");
      else if (!g_strcasecmp(slant, "o"))
        slant = N_("oblique");
      else if (!g_strcasecmp(slant, "ri"))
        slant = N_("reverse italic");
      else if (!g_strcasecmp(slant, "ro"))
        slant = N_("reverse oblique");
      else if (!g_strcasecmp(slant, "ot"))
        slant = N_("other");

      if (!g_strcasecmp(set_width, "normal"))
        set_width = NULL;
      else if (!g_strcasecmp(set_width, "(nil)"))
        set_width = NULL;

      if (!g_strcasecmp(spacing, "p"))
        spacing = NULL;
      else if (!g_strcasecmp(spacing, "(nil)"))
        spacing = NULL;
      else if (!g_strcasecmp(spacing, "m"))
        spacing = N_("[M]");
      else if (!g_strcasecmp(spacing, "c"))
        spacing = N_("[C]");
      ...
      new_item = buffer;
      row = gtk_clist_append(GTK_CLIST(fontsel->font_style_clist), &new_item);
      if (show_charset) gtk_clist_set_shift(GTK_CLIST(fontsel->font_style_clist), row, 0, 0, 4);
      gtk_clist_set_row_data(GTK_CLIST(fontsel->font_style_clist), row, GINT_TO_POINTER(tmpstyle));
    }
  }
...
static void gtk_xlfd_selection_select_best_style(GtkXlfdSelection *fontsel, gboolean use_first) {
  FontInfo *font;
  FontStyle *styles;
  ...
  for (row = 0; row < GTK_CLIST(fontsel->font_style_clist)->rows; row++) {
    ...
    for (prop = 0; prop < GTK_NUM_STYLE_PROPERTIES; prop++) {
      if (fontsel->property_values[prop] == styles[style].properties[prop]) matched++;
    }
  ...
  if (gtk_clist_row_is_visible(GTK_CLIST(fontsel->font_style_clist), best_row) !=
      GTK_VISIBILITY_FULL)
    gtk_clist_moveto(GTK_CLIST(fontsel->font_style_clist), best_row, -1, 0.5, 0);
  gtk_xlfd_selection_show_available_sizes(fontsel);
  gtk_xlfd_selection_select_best_size(fontsel);
}
...
static void gtk_xlfd_selection_show_available_sizes(GtkXlfdSelection *fontsel) {
  FontInfo *font;
  ...
  if (fontsel->metric == GTK_XLFD_METRIC_POINTS) {
  ...
  type_filter = fontsel->filters[GTK_XLFD_FILTER_BASE].font_type &
                fontsel->filters[GTK_XLFD_FILTER_USER].font_type;

  if (!((style->flags & GTK_XLFD_SCALABLE_BITMAP && type_filter & GTK_XLFD_SCALABLE_BITMAP) ||
        (style->flags & GTK_XLFD_SCALABLE && type_filter & GTK_XLFD_SCALABLE)))
    nstandard_sizes = 0;
  ...
  while (nstandard_sizes || nbitmapped_sizes) {
    ...
    if (nbitmapped_sizes) {
      if (fontsel->metric == GTK_XLFD_METRIC_POINTS) {
        if (*bitmapped_sizes % 10 != 0) can_match = FALSE;
    ...
    if (can_match && nstandard_sizes && nbitmapped_sizes && *standard_sizes == bitmap_size) {
      ...
      nbitmapped_sizes--;
               ...
               (!nbitmapped_sizes || (gfloat)*standard_sizes < bitmap_size_float)) {
      sprintf(buffer, "%i", *standard_sizes);
      standard_sizes++;
      nstandard_sizes--;
    } else {
      if (fontsel->metric == GTK_XLFD_METRIC_POINTS) {
        if (*bitmapped_sizes % 10 == 0)
          ...
          sprintf(buffer, "%i.%i *", *bitmapped_sizes / 10, *bitmapped_sizes % 10);
      } else {
...
static void gtk_xlfd_selection_update_size(GtkXlfdSelection *fontsel) {
  gint new_size;
  ...
  if (fontsel->metric == GTK_XLFD_METRIC_PIXELS) {
    new_size = atoi(text);
...
/* If the user hits return in the font size entry, we change to the new font
   size. */
static void gtk_xlfd_selection_size_activate(GtkWidget *w, gpointer data) {
  gtk_xlfd_selection_update_size(data);
}
...
/* This tries to select the closest size to the current size, though it
   ...
   Note: this will load a font. */
static void gtk_xlfd_selection_select_best_size(GtkXlfdSelection *fontsel) {
  FontInfo *font;
  FontStyle *styles, *style;
  gchar *text;
  ...
  if (fontsel->font_index == -1) return;
  ...
  for (row = 0; row < GTK_CLIST(fontsel->size_clist)->rows; row++) {
    ...
    if (fontsel->metric == GTK_XLFD_METRIC_POINTS) {
      size *= 10;
      if (nmatched == 2) size += size_fraction;
    }
    ...
    if (size == fontsel->selected_size) {
      ...
      break;
               ...
               abs(size - fontsel->selected_size) < (abs(best_size - fontsel->selected_size))) {
      best_size = size;
      best_row = row;
    }
  }
  ...
  if (found) {
    ...
    selection = GTK_CLIST(fontsel->size_clist)->selection;
    if (selection)
      gtk_clist_unselect_row(GTK_CLIST(fontsel->size_clist), GPOINTER_TO_INT(selection->data), 0);
    gtk_clist_moveto(GTK_CLIST(fontsel->size_clist), best_row, -1, 0.5, 0);
  ...
  gtk_xlfd_selection_load_font(fontsel);
}
...
static void gtk_xlfd_selection_select_size(GtkWidget *w, gint row, gint column,
  ...
  /* Check if the font size has changed, and return if it hasn't. */
  new_size = atof(text);
  if (fontsel->metric == GTK_XLFD_METRIC_POINTS) new_size *= 10;

  if (fontsel->size == (gint)new_size) return;
...
static void gtk_xlfd_selection_metric_callback(GtkWidget *w, gpointer data) {
  GtkXlfdSelection *fontsel = GTK_XLFD_SELECTION(data);

#ifdef FONTSEL_DEBUG
  g_message("In metric_callback\n");
#endif
  if (GTK_TOGGLE_BUTTON(fontsel->pixels_button)->active) {
    if (fontsel->metric == GTK_XLFD_METRIC_PIXELS) return;
    fontsel->metric = GTK_XLFD_METRIC_PIXELS;
    fontsel->size = (fontsel->size + 5) / 10;
    fontsel->selected_size = (fontsel->selected_size + 5) / 10;
  } else {
    if (fontsel->metric == GTK_XLFD_METRIC_POINTS) return;
    fontsel->metric = GTK_XLFD_METRIC_POINTS;
    fontsel->size *= 10;
...
static guint16 gtk_xlfd_selection_field_to_index(gchar **table, gint ntable, gchar *field) {
  ...
  for (i = 0; i < ntable; i++)
    if (strcmp(field, table[i]) == 0) return i;

  return 0;
}

/* This attempts to load the current font, and returns TRUE if it succeeds. */
static gboolean gtk_xlfd_selection_load_font(GtkXlfdSelection *fontsel) {
  GdkFont *font;
  ...
  fontname = gtk_xlfd_selection_get_font_name(fontsel);
  if (fontname) {
#ifdef FONTSEL_DEBUG
    g_message("Loading: %s\n", fontname);
#endif
    ...
    if (font) {
      ...
      if (strcmp(label_text, "")) gtk_label_set_text(GTK_LABEL(fontsel->message_label), "");
      gtk_xlfd_selection_update_preview(fontsel);
      return TRUE;
    } else {
...
static void gtk_xlfd_selection_update_preview(GtkXlfdSelection *fontsel) {
  ...
  if (new_height < INITIAL_PREVIEW_HEIGHT) new_height = INITIAL_PREVIEW_HEIGHT;
  if (new_height > MAX_PREVIEW_HEIGHT) new_height = MAX_PREVIEW_HEIGHT;
  ...
  if (strlen(text) == 0) gtk_entry_set_text(GTK_ENTRY(fontsel->preview_entry), PREVIEW_TEXT);
...
static void gtk_xlfd_selection_switch_page(GtkWidget *w, GtkNotebookPage *page, gint page_num,
  ...
  if (page_num == 0)
    gtk_xlfd_selection_update_filter(fontsel);
  else if (page_num == 1)
    gtk_xlfd_selection_show_font_info(fontsel);
}

static void gtk_xlfd_selection_show_font_info(GtkXlfdSelection *fontsel) {
  Atom font_atom, atom;
  Bool status;
  char *name;
  gchar *fontname;
  ...
  fontname = gtk_xlfd_selection_get_font_name(fontsel);
  gtk_entry_set_text(GTK_ENTRY(fontsel->requested_font_name), fontname ? fontname : "");

  gtk_clist_freeze(GTK_CLIST(fontsel->info_clist));
  for (i = 0; i < GTK_XLFD_NUM_FIELDS; i++) {
    if (fontname)
      field = gtk_xlfd_selection_get_xlfd_field(fontname, i, field_buffer);
    else
      field = NULL;
    if (field) {
      if (i == XLFD_SLANT)
        field = gtk_xlfd_selection_expand_slant_code(field);
      else if (i == XLFD_SPACING)
        field = gtk_xlfd_selection_expand_spacing_code(field);
    }
    gtk_clist_set_text(GTK_CLIST(fontsel->info_clist), i, 1, field ? field : "");
  }
  ...
  if (fontsel->font) {
    ...
    if (fontsel->font->type == GDK_FONT_FONTSET) {
      ...
      num_fonts = XFontsOfFontSet(GDK_FONT_XFONT(fontsel->font), &font_structs, &font_names);

      status = XGetFontProperty(font_structs[0], font_atom, &atom);
    } else {
      status = XGetFontProperty(GDK_FONT_XFONT(fontsel->font), font_atom, &atom);
    }

    if (status == True) {
      name = gdk_atom_name(
          gdk_x11_xatom_to_atom_for_display(gtk_widget_get_display(GTK_WIDGET(fontsel)), atom));
      gtk_entry_set_text(GTK_ENTRY(fontsel->actual_font_name), name);

      for (i = 0; i < GTK_XLFD_NUM_FIELDS; i++) {
        field = gtk_xlfd_selection_get_xlfd_field(name, i, field_buffer);
        if (i == XLFD_SLANT)
          field = gtk_xlfd_selection_expand_slant_code(field);
        else if (i == XLFD_SPACING)
          field = gtk_xlfd_selection_expand_spacing_code(field);
        gtk_clist_set_text(GTK_CLIST(fontsel->info_clist), i, 2, field ? field : "");
      }
  ...
  if (!shown_actual_fields) {
    ...
    for (i = 0; i < GTK_XLFD_NUM_FIELDS; i++) {
      gtk_clist_set_text(GTK_CLIST(fontsel->info_clist), i, 2, fontname ? _("(unknown)") : "");
    }
  }
  gtk_clist_thaw(GTK_CLIST(fontsel->info_clist));
  g_free(fontname);
...
static gchar *gtk_xlfd_selection_expand_slant_code(gchar *slant) {
  if (!g_strcasecmp(slant, "r"))
    ...
    return (_("reverse italic"));
  else if (!g_strcasecmp(slant, "ro"))
    return (_("reverse oblique"));
  else if (!g_strcasecmp(slant, "ot"))
    return (_("other"));
  return slant;
}
...
static void gtk_xlfd_selection_select_filter(GtkWidget *w, gint row, gint column,
                                             GdkEventButton *bevent, GtkXlfdSelection *fontsel) {
  gint i, prop, index;

  if (row == 0) {
    for (i = 1; i < GTK_CLIST(w)->rows; i++) gtk_clist_unselect_row(GTK_CLIST(w), i, 0);
  } else {
    /* Find out which property this is. */
    for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++)
      if (fontsel->filter_clists[prop] == w) break;
    index = GPOINTER_TO_INT(gtk_clist_get_row_data(GTK_CLIST(w), row));
    if (gtk_xlfd_selection_filter_state(fontsel, GTK_XLFD_FILTER_BASE, prop, index) ==
        NOT_FILTERED) {
...
/* This is called when the main notebook page is selected. It checks if the
   ...
   turned off. */
static void gtk_xlfd_selection_update_filter(GtkXlfdSelection *fontsel) {
  GtkWidget *clist;
  GList *selection;
  gboolean default_filter = TRUE, filter_changed = FALSE;
  gint prop, nselected, i, row, index;
  GtkXlfdFilter *filter = &fontsel->filters[GTK_XLFD_FILTER_USER];
  ...
  base_font_type = fontsel->filters[GTK_XLFD_FILTER_BASE].font_type & GTK_XLFD_ALL;
  user_font_type = fontsel->filters[GTK_XLFD_FILTER_USER].font_type & GTK_XLFD_ALL;
  new_font_type = GTK_TOGGLE_BUTTON(fontsel->type_bitmaps_button)->active ? GTK_XLFD_BITMAP : 0;
  new_font_type |=
      (GTK_TOGGLE_BUTTON(fontsel->type_scalable_button)->active ? GTK_XLFD_SCALABLE : 0);
  new_font_type |=
      (GTK_TOGGLE_BUTTON(fontsel->type_scaled_bitmaps_button)->active ? GTK_XLFD_SCALABLE_BITMAP
                                                                      : 0);
  new_font_type &= base_font_type;
  new_font_type |= (~base_font_type & user_font_type);
  if (new_font_type != (GTK_XLFD_BITMAP | GTK_XLFD_SCALABLE)) default_filter = FALSE;

  if (new_font_type != user_font_type) filter_changed = TRUE;
  fontsel->filters[GTK_XLFD_FILTER_USER].font_type = new_font_type;
    ...
    if (nselected != 1 || GPOINTER_TO_INT(selection->data) != 0) {
      ...
      if (filter->property_nfilters[prop] != nselected)
        ...
        for (i = 0; i < nselected; i++) {
          ...
          if (filter->property_filters[prop][i] != index) filter_changed = TRUE;
          selection = selection->next;
        }
      }
    } else {
      if (filter->property_nfilters[prop] != 0) filter_changed = TRUE;
    }
  }
...
static void gtk_xlfd_selection_show_available_fonts(GtkXlfdSelection *fontsel) {
  FontInfo *font_info, *font;
  GtkXlfdFilter *filter;
  ...
  for (i = 0; i < nfonts; i++) {
    ...
    if (fontsel->font_index == i) font_row = row;
  }
  ...
  if (gtk_clist_row_is_visible(GTK_CLIST(fontsel->font_clist), font_row) != GTK_VISIBILITY_FULL)
    gtk_clist_moveto(GTK_CLIST(fontsel->font_clist), font_row, -1, 0.5, 0);

  gtk_xlfd_selection_show_available_styles(fontsel);
  gtk_xlfd_selection_select_best_style(fontsel, FALSE);
}

/* Returns TRUE if the style is not currently filtered out. */
static gboolean gtk_xlfd_selection_style_visible(GtkXlfdSelection *fontsel, FontInfo *font,
                                                 gint style_index) {
  FontStyle *styles, *style;
  GtkXlfdFilter *filter;
  guint16 value;
  ...
  type_filter = fontsel->filters[GTK_XLFD_FILTER_BASE].font_type &
                fontsel->filters[GTK_XLFD_FILTER_USER].font_type;
  if (!(style->flags & type_filter)) return FALSE;
...
static void gtk_xlfd_selection_reset_filter(GtkWidget *w, GtkXlfdSelection *fontsel) {
  gint prop, base_font_type;

  fontsel->filters[GTK_XLFD_FILTER_USER].font_type = GTK_XLFD_BITMAP | GTK_XLFD_SCALABLE;

  base_font_type = fontsel->filters[GTK_XLFD_FILTER_BASE].font_type;
  if (base_font_type & GTK_XLFD_BITMAP)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_bitmaps_button), TRUE);
  if (base_font_type & GTK_XLFD_SCALABLE)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_scalable_button), TRUE);
  if (base_font_type & GTK_XLFD_SCALABLE_BITMAP)
...
static void gtk_xlfd_selection_on_clear_filter(GtkWidget *w, GtkXlfdSelection *fontsel) {
  gtk_xlfd_selection_clear_filter(fontsel);
}
...
static void gtk_xlfd_selection_clear_filter(GtkXlfdSelection *fontsel) {
  GtkXlfdFilter *filter;
  gint prop;
  ...
  filter = &fontsel->filters[GTK_XLFD_FILTER_USER];
  filter->font_type = GTK_XLFD_BITMAP | GTK_XLFD_SCALABLE;
  for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++) {
    g_free(filter->property_filters[prop]);
    filter->property_filters[prop] = NULL;
    filter->property_nfilters[prop] = 0;
  ...
  gtk_xlfd_selection_reset_filter(NULL, fontsel);
...
void gtk_xlfd_selection_set_filter(GtkXlfdSelection *fontsel, GtkXlfdFilterType filter_type,
                                   GtkXlfdType font_type, gchar **foundries, gchar **weights,
  ...
  for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++) {
    ...
    if (filter_strings[prop]) {
      ...
      nfilters = 0;
      while (filter_strings[prop][nfilters]) nfilters++;
      ...
      for (i = 0; i < nfilters; i++) {
        ...
        for (j = 0; j < fontsel_info->nproperties[prop]; j++) {
          ...
          property_alt = NULL;
          if (prop == SLANT)
            property_alt = gtk_xlfd_selection_expand_slant_code(property);
          else if (prop == SPACING)
            property_alt = gtk_xlfd_selection_expand_spacing_code(property);
  ...
  gtk_xlfd_selection_update_filter_lists(fontsel);

  if (filter_type == GTK_XLFD_FILTER_BASE) {
    user_font_type = fontsel->filters[GTK_XLFD_FILTER_USER].font_type;
    if (font_type & GTK_XLFD_BITMAP) {
      gtk_widget_set_sensitive(fontsel->type_bitmaps_button, TRUE);
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_bitmaps_button),
                                   user_font_type & GTK_XLFD_BITMAP);
    } else {
    ...
    if (font_type & GTK_XLFD_SCALABLE) {
      gtk_widget_set_sensitive(fontsel->type_scalable_button, TRUE);
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_scalable_button),
                                   user_font_type & GTK_XLFD_SCALABLE);
    } else {
    ...
    if (font_type & GTK_XLFD_SCALABLE_BITMAP) {
      gtk_widget_set_sensitive(fontsel->type_scaled_bitmaps_button, TRUE);
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_scaled_bitmaps_button),
                                   user_font_type & GTK_XLFD_SCALABLE_BITMAP);
    } else {
    ...
    if (base_font_type & GTK_XLFD_BITMAP)
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_bitmaps_button),
                                   font_type & GTK_XLFD_BITMAP);

    if (base_font_type & GTK_XLFD_SCALABLE)
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_scalable_button),
                                   font_type & GTK_XLFD_SCALABLE);

    if (base_font_type & GTK_XLFD_SCALABLE_BITMAP)
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(fontsel->type_scaled_bitmaps_button),
                                   font_type & GTK_XLFD_SCALABLE_BITMAP);

    /* If the user filter is not the default, make the 'Reset Filter' button
       sensitive. */
    filter_set = FALSE;
    if (font_type != (GTK_XLFD_BITMAP | GTK_XLFD_SCALABLE)) filter_set = TRUE;
    for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++) {
      if (filter->property_nfilters[prop] != 0) filter_set = TRUE;
    }
    if (filter_set) gtk_widget_set_sensitive(fontsel->filter_button, TRUE);
  }
...
static void gtk_xlfd_selection_update_filter_lists(GtkXlfdSelection *fontsel) {
  GtkWidget *clist;
  ...
  for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++) {
    ...
    for (row = 1; row < GTK_CLIST(clist)->rows; row++) {
      ...
      if (gtk_xlfd_selection_filter_state(fontsel, GTK_XLFD_FILTER_USER, prop, index) == FILTERED &&
          fg == NULL)
        gtk_clist_select_row(GTK_CLIST(clist), row, 0);
      else
...
static GtkXlfdPropertyFilterState gtk_xlfd_selection_filter_state(GtkXlfdSelection *fontsel,
                                                                  GtkXlfdFilterType filter_type,
                                                                  gint property, gint index) {
  GtkXlfdFilter *filter;
  gint i;
  ...
  filter = &fontsel->filters[filter_type];
  if (filter->property_nfilters[property] == 0) return NOT_SET;

  for (i = 0; i < filter->property_nfilters[property]; i++) {
    if (filter->property_filters[property][i] == index) return FILTERED;
  }
  return NOT_FILTERED;
}
...
static void gtk_xlfd_selection_get_fonts(void) {
  ...
  fontsel_info = g_new(GtkXlfdSelInfo, 1);
  ...
  if (num_fonts == MAX_FONTS) g_warning(_("MAX_FONTS exceeded. Some fonts may be missing."));
  ...
  for (i = 0; i < num_fonts; i++) {
    ...
    g_message("%s\n", xfontnames[i]);
#endif
    if (gtk_xlfd_selection_is_xlfd_font_name(xfontnames[i]))
      gtk_xlfd_selection_insert_font(fontnames, &fontsel_info->nfonts, xfontnames[i]);
    else {
#ifdef FONTSEL_DEBUG
  ...
  /* Since many font names will be in the same FontInfo not all of the
     ...
     with the real size. */
  fontsel_info->font_info =
      g_realloc(fontsel_info->font_info, sizeof(FontInfo) * fontsel_info->nfonts);
  ...
  for (i = 0; i < fontsel_info->nfonts; i++) {
    ...
    while (temp_list) {
      ...
      for (prop = 0; prop < GTK_NUM_STYLE_PROPERTIES; prop++) {
        current_style->properties[prop] = gtk_xlfd_selection_insert_field(fontname, prop);
      }
      ...
      field = gtk_xlfd_selection_get_xlfd_field(fontname, XLFD_PIXELS, field_buffer);
      pixels = atoi(field);

      field = gtk_xlfd_selection_get_xlfd_field(fontname, XLFD_POINTS, field_buffer);
      points = atoi(field);

      field = gtk_xlfd_selection_get_xlfd_field(fontname, XLFD_RESOLUTION_X, field_buffer);
      res_x = atoi(field);

      field = gtk_xlfd_selection_get_xlfd_field(fontname, XLFD_RESOLUTION_Y, field_buffer);
      res_y = atoi(field);

      if (pixels == 0 && points == 0) {
        if (res_x == 0 && res_y == 0)
          flags = GTK_XLFD_SCALABLE;
        else
          flags = GTK_XLFD_SCALABLE_BITMAP;
      } else
        flags = GTK_XLFD_BITMAP;
      ...
      while (prev_style < current_style) {
        matched_style = TRUE;
        for (prop = 0; prop < GTK_NUM_STYLE_PROPERTIES; prop++) {
          if (prev_style->properties[prop] != current_style->properties[prop]) {
            matched_style = FALSE;
            break;
          }
        }
        if (matched_style) break;
        prev_style++;
      }
      ...
      if (matched_style) {
        ...
        if (flags == GTK_XLFD_BITMAP) {
          pixel_sizes = fontsel_info->pixel_sizes + prev_style->pixel_sizes_index;
          found_size = FALSE;
          ...
          if (!found_size) {
            for (tmp_sizes = fontsel_info->pixel_sizes + npixel_sizes; tmp_sizes > pixel_sizes;
                 tmp_sizes--)
          ...
          point_sizes = fontsel_info->point_sizes + prev_style->point_sizes_index;
          found_size = FALSE;
          ...
          /* We need to move all the following point sizes up, and also
             update the indexes of any following styles. */
          if (!found_size) {
            for (tmp_sizes = fontsel_info->point_sizes + npoint_sizes; tmp_sizes > point_sizes;
                 tmp_sizes--)
              *tmp_sizes = *(tmp_sizes - 1);
  ...
  fontsel_info->font_styles =
      g_realloc(fontsel_info->font_styles, sizeof(FontStyle) * fontsel_info->nstyles);
  fontsel_info->pixel_sizes = g_realloc(fontsel_info->pixel_sizes, sizeof(guint16) * npixel_sizes);
  fontsel_info->point_sizes = g_realloc(fontsel_info->point_sizes, sizeof(guint16) * npoint_sizes);
  g_free(fontnames);
...
#ifdef FONTSEL_DEBUG
  g_message(
      "\n\n Font Family           Weight    Slant     Set Width Spacing   "
      "Charset\n\n");
  for (i = 0; i < fontsel_info->nfonts; i++) {
    ...
    for (style = 0; style < font->nstyles; style++) {
      ...
      for (prop = 0; prop < GTK_NUM_STYLE_PROPERTIES; prop++)
        g_message("%-9.9s ", fontsel_info->properties[prop][styles->properties[prop]]);
      g_message("\n      ");

      if (styles->flags & GTK_XLFD_BITMAP) g_message("Bitmapped font  ");
      if (styles->flags & GTK_XLFD_SCALABLE) g_message("Scalable font  ");
      if (styles->flags & GTK_XLFD_SCALABLE_BITMAP) g_message("Scalable-Bitmapped font  ");
      g_message("\n");

      if (styles->npixel_sizes) {
        g_message("      Pixel sizes: ");
        tmp_sizes = fontsel_info->pixel_sizes + styles->pixel_sizes_index;
        for (size = 0; size < styles->npixel_sizes; size++) g_message("%i ", *tmp_sizes++);
        g_message("\n");
      }
      ...
      if (styles->npoint_sizes) {
        ...
        for (size = 0; size < styles->npoint_sizes; size++) g_message("%i ", *tmp_sizes++);
        g_message("\n");
      }
...
static void gtk_xlfd_selection_insert_font(GSList *fontnames[], gint *ntable, gchar *fontname) {
  ...
  family = gtk_xlfd_selection_get_xlfd_field(fontname, XLFD_FAMILY, family_buffer);
  if (!family) return;

  foundry = gtk_xlfd_selection_insert_field(fontname, FOUNDRY);
  ...
  if (*ntable > 0) {
    ...
    while (lower < upper) {
      ...
      if (cmp == 0) {
        fontnames[middle] = g_slist_prepend(fontnames[middle], fontname);
        return;
      } else if (cmp < 0)
        upper = middle;
      else
...
static gint gtk_xlfd_selection_insert_field(gchar *fontname, gint prop) {
  ...
  field = gtk_xlfd_selection_get_xlfd_field(fontname, xlfd_index[prop], field_buffer);
  if (!field) return 0;

  /* If the field is already in the array just return its index. */
  for (index = 0; index < fontsel_info->nproperties[prop]; index++)
    if (!strcmp(field, fontsel_info->properties[prop][index])) return index;

  /* Make sure we have enough space to add the field. */
  if (fontsel_info->nproperties[prop] == fontsel_info->space_allocated[prop]) {
    fontsel_info->space_allocated[prop] += PROPERTY_ARRAY_INCREMENT;
    fontsel_info->properties[prop] = g_realloc(
        fontsel_info->properties[prop], sizeof(gchar *) * fontsel_info->space_allocated[prop]);
  }
...
GdkFont *gtk_xlfd_selection_get_font(GtkXlfdSelection *fontsel) {
  g_return_val_if_fail(GTK_IS_XLFD_SELECTION(fontsel), NULL);

  gtk_xlfd_selection_update_size(fontsel);
...
gchar *gtk_xlfd_selection_get_font_name(GtkXlfdSelection *fontsel) {
  ...
  if (strcmp(foundry_str, "(nil)") == 0) foundry_str = "";
  ...
  return gtk_xlfd_selection_create_xlfd(
      fontsel->size, fontsel->metric, foundry_str, family_str, property_str[WEIGHT],
      property_str[SLANT], property_str[SET_WIDTH], property_str[SPACING], property_str[CHARSET]);
}
...
gboolean gtk_xlfd_selection_set_font_name(GtkXlfdSelection *fontsel, const gchar *fontname) {
  gchar *family, *field;
  ...
  g_return_val_if_fail(GTK_IS_XLFD_SELECTION(fontsel), FALSE);
  g_return_val_if_fail(fontname != NULL, FALSE);

  /* Check it is a valid fontname. */
  if (!gtk_xlfd_selection_is_xlfd_font_name(fontname)) return FALSE;

  family = gtk_xlfd_selection_get_xlfd_field(fontname, XLFD_FAMILY, family_buffer);
  if (!family) return FALSE;

  field = gtk_xlfd_selection_get_xlfd_field(fontname, XLFD_FOUNDRY, field_buffer);
  foundry = gtk_xlfd_selection_field_to_index(fontsel_info->properties[FOUNDRY],
                                              fontsel_info->nproperties[FOUNDRY], field);

  index = gtk_xlfd_selection_find_font(fontsel, family, foundry);
  if (index == -1) return FALSE;

  /* Convert the property fields into indices and set them. */
  for (prop = 0; prop < GTK_NUM_STYLE_PROPERTIES; prop++) {
    field = gtk_xlfd_selection_get_xlfd_field(fontname, xlfd_index[prop], field_buffer);
    value = gtk_xlfd_selection_field_to_index(fontsel_info->properties[prop],
                                              fontsel_info->nproperties[prop], field);
  ...
  row = gtk_clist_find_row_from_data(GTK_CLIST(fontsel->font_clist), GINT_TO_POINTER(index));
  if (row != -1) {
...
static gint gtk_xlfd_selection_find_font(GtkXlfdSelection *fontsel, gchar *family,
                                         guint16 foundry) {
  FontInfo *font_info;
...
void gtk_xlfd_selection_set_preview_text(GtkXlfdSelection *fontsel, const gchar *text) {
 ...
 */
static gchar *gtk_xlfd_selection_get_xlfd_field(const gchar *fontname, FontField field_num,
                                                gchar *buffer) {
  const gchar *t1, *t2;
...
static gchar *gtk_xlfd_selection_create_xlfd(gint size, GtkXlfdMetricType metric, gchar *foundry,
  ...
  if (metric == GTK_XLFD_METRIC_PIXELS)
    pixel_size = buffer;
  else
    point_size = buffer;

  fontname = g_strdup_printf("-%s-%s-%s-%s-%s-*-%s-%s-*-*-%s-*-%s", foundry, family, weight, slant,
                             set_width, pixel_size, point_size, spacing, charset);
  return fontname;
}

/*****************************************************************************
 * GtkXlfdSelectionDialog
 *****************************************************************************/
...
guint gtk_xlfd_selection_dialog_get_type(void) {
  ...
  if (!font_selection_dialog_type) {
    GtkTypeInfo fontsel_diag_info = {
        "GtkXlfdSelectionDialog", sizeof(GtkXlfdSelectionDialog),
        sizeof(GtkXlfdSelectionDialogClass), (GtkClassInitFunc)gtk_xlfd_selection_dialog_class_init,
        (GtkObjectInitFunc)gtk_xlfd_selection_dialog_init,
        /* reserved_1 */ NULL,
        /* reserved_2 */ NULL, (GtkClassInitFunc)NULL,
    };
...
static void gtk_xlfd_selection_dialog_init(GtkXlfdSelectionDialog *fontseldiag) {
  ...
  gtk_signal_connect(GTK_OBJECT(fontseldiag), "configure_event",
                     (GtkSignalFunc)gtk_xlfd_selection_dialog_on_configure, fontseldiag);
  ...
  fontseldiag->fontsel = gtk_xlfd_selection_new();
  gtk_widget_show(fontseldiag->fontsel);
  gtk_box_pack_start(GTK_BOX(fontseldiag->main_vbox), fontseldiag->fontsel, TRUE, TRUE, 0);

  /* Create the action area */
  fontseldiag->action_area = gtk_hbutton_box_new();
  gtk_button_box_set_layout(GTK_BUTTON_BOX(fontseldiag->action_area), GTK_BUTTONBOX_END);
  gtk_button_box_set_spacing(GTK_BUTTON_BOX(fontseldiag->action_area), 5);
  gtk_box_pack_start(GTK_BOX(fontseldiag->main_vbox), fontseldiag->action_area, FALSE, FALSE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(fontseldiag->action_area), fontseldiag->ok_button, TRUE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(fontseldiag->action_area), fontseldiag->apply_button, TRUE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(fontseldiag->action_area), fontseldiag->cancel_button, TRUE, TRUE, 0);
}

GtkWidget *gtk_xlfd_selection_dialog_new(const gchar *title) {
  GtkXlfdSelectionDialog *fontseldiag;

  fontseldiag = gtk_type_new(GTK_TYPE_XLFD_SELECTION_DIALOG);
...
gchar *gtk_xlfd_selection_dialog_get_font_name(GtkXlfdSelectionDialog *fsd) {
  return gtk_xlfd_selection_get_font_name(GTK_XLFD_SELECTION(fsd->fontsel));
}

GdkFont *gtk_xlfd_selection_dialog_get_font(GtkXlfdSelectionDialog *fsd) {
  return gtk_xlfd_selection_get_font(GTK_XLFD_SELECTION(fsd->fontsel));
}

gboolean gtk_xlfd_selection_dialog_set_font_name(GtkXlfdSelectionDialog *fsd,
                                                 const gchar *fontname) {
  return gtk_xlfd_selection_set_font_name(GTK_XLFD_SELECTION(fsd->fontsel), fontname);
}

void gtk_xlfd_selection_dialog_set_filter(GtkXlfdSelectionDialog *fsd,
                                          GtkXlfdFilterType filter_type, GtkXlfdType font_type,
                                          gchar **foundries, gchar **weights, gchar **slants,
                                          gchar **setwidths, gchar **spacings, gchar **charsets) {
  gtk_xlfd_selection_set_filter(GTK_XLFD_SELECTION(fsd->fontsel), filter_type, font_type, foundries,
...
void gtk_xlfd_selection_dialog_set_preview_text(GtkXlfdSelectionDialog *fsd, const gchar *text) {
  gtk_xlfd_selection_set_preview_text(GTK_XLFD_SELECTION(fsd->fontsel), text);
}
