...
static void    gtk_xlfd_selection_insert_font        (GSList         *fontnames[],
						      gint           *ntable,
						      gchar          *fontname);
static gint    gtk_xlfd_selection_insert_field       (gchar          *fontname,
						      gint            prop);

/* These are the callbacks & related functions. */
static void    gtk_xlfd_selection_select_font	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      gpointer        data);
static gint    gtk_xlfd_selection_on_clist_key_press (GtkWidget      *clist,
						      GdkEventKey    *event,
						      GtkXlfdSelection *fs);
static gboolean gtk_xlfd_selection_select_next	     (GtkXlfdSelection *fs,
						      GtkWidget        *clist,
						      gint		step);
static void    gtk_xlfd_selection_show_available_styles
(GtkXlfdSelection *fs);
static void    gtk_xlfd_selection_select_best_style  (GtkXlfdSelection *fs,
						      gboolean	       use_first);

static void    gtk_xlfd_selection_select_style	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      gpointer        data);
static void    gtk_xlfd_selection_show_available_sizes
(GtkXlfdSelection *fs);
static void     gtk_xlfd_selection_size_activate  (GtkWidget *w,
						   gpointer   data);

static void    gtk_xlfd_selection_select_best_size   (GtkXlfdSelection *fs);
static void    gtk_xlfd_selection_select_size	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      gpointer        data);

static void    gtk_xlfd_selection_metric_callback    (GtkWidget      *w,
						      gpointer        data);
static void    gtk_xlfd_selection_expose_list	     (GtkWidget	     *w,
						      GdkEventExpose *event,
						      gpointer        data);
static void    gtk_xlfd_selection_realize_list	     (GtkWidget	     *widget,
						      gpointer	      data);

static void    gtk_xlfd_selection_switch_page	     (GtkWidget      *w,
						      GtkNotebookPage *page,
						      gint             page_num,
						      gpointer         data);
static void    gtk_xlfd_selection_show_font_info     (GtkXlfdSelection *fs);

static void    gtk_xlfd_selection_select_filter	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      GtkXlfdSelection *fs);
static void    gtk_xlfd_selection_update_filter	     (GtkXlfdSelection *fs);
static gboolean gtk_xlfd_selection_style_visible     (GtkXlfdSelection *fs,
						      FontInfo       *font,
						      gint	      style);
static void    gtk_xlfd_selection_reset_filter	     (GtkWidget      *w,
						      GtkXlfdSelection *fs);
static void    gtk_xlfd_selection_on_clear_filter    (GtkWidget      *w,
						      GtkXlfdSelection *fs);
static void    gtk_xlfd_selection_show_available_fonts
						     (GtkXlfdSelection *fs);
...
static GtkXlfdPropertyFilterState gtk_xlfd_selection_filter_state
						     (GtkXlfdSelection *fs,
...
static gint    gtk_xlfd_selection_find_font	     (GtkXlfdSelection *fs,
						      gchar          *family,
						      guint16	      foundry);
static guint16 gtk_xlfd_selection_field_to_index     (gchar         **table,
						      gint            ntable,
						      gchar          *field);
...
static char*   gtk_xlfd_selection_get_xlfd_field     (const gchar    *fontname,
						      FontField       field_num,
						      gchar          *buffer);
static gchar * gtk_xlfd_selection_create_xlfd        (gint            size,
...
static gint    gtk_xlfd_selection_dialog_on_configure(GtkWidget      *widget,
  ...
  if(!font_selection_type)
    {
      ...
      {
	"GtkXlfdSelection",
	sizeof (GtkXlfdSelection),
	sizeof (GtkXlfdSelectionClass),
	(GtkClassInitFunc) gtk_xlfd_selection_class_init,
	(GtkObjectInitFunc) gtk_xlfd_selection_init,
	/* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
...
static void
gtk_xlfd_selection_class_init(GtkXlfdSelectionClass *klass)
{
...
static void
gtk_xlfd_selection_init(GtkXlfdSelection *fontsel)
{
  ...
  fontsel->filters[GTK_XLFD_FILTER_USER].font_type = GTK_XLFD_BITMAP
    | GTK_XLFD_SCALABLE;
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel->size_entry), "activate",
		      GTK_SIGNAL_FUNC (gtk_xlfd_selection_size_activate),
		      fontsel);
  ...
  gtk_signal_connect_after (GTK_OBJECT (fontsel->font_style_clist),
			    "realize",
			    GTK_SIGNAL_FUNC(gtk_xlfd_selection_realize_list),
			    fontsel);
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel->size_clist), "select_row",
		      GTK_SIGNAL_FUNC(gtk_xlfd_selection_select_size),
		      fontsel);
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel->filter_button), "clicked",
		      GTK_SIGNAL_FUNC(gtk_xlfd_selection_on_clear_filter),
		      fontsel);
  ...
  gtk_signal_connect(GTK_OBJECT(fontsel->points_button), "toggled",
		     (GtkSignalFunc) gtk_xlfd_selection_metric_callback,
		     fontsel);
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel), "switch_page",
		      GTK_SIGNAL_FUNC(gtk_xlfd_selection_switch_page),
		      fontsel);
  ...
  for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++)
      ...
      if (top == 2 && left == 2)
	{
	  ...
	  gtk_signal_connect (GTK_OBJECT (button), "clicked",
			      GTK_SIGNAL_FUNC(gtk_xlfd_selection_reset_filter),
			      fontsel);
	}
      else
	gtk_table_attach (GTK_TABLE (table), scrolled_win,
      ...
      gtk_signal_connect (GTK_OBJECT (clist), "select_row",
			  GTK_SIGNAL_FUNC(gtk_xlfd_selection_select_filter),
			  fontsel);
...
static void
gtk_xlfd_selection_destroy (GtkObject *object)
{
  GtkXlfdSelection *fontsel;
...
/* This is called when the clist is exposed. Here we scroll to the current
   font if necessary. */
static void
gtk_xlfd_selection_expose_list (GtkWidget		*widget,
				GdkEventExpose		*event,
				gpointer		 data)
{
  GtkXlfdSelection *fontsel;
...
static void
gtk_xlfd_selection_realize_list (GtkWidget		*widget,
				 gpointer		 data)
{
  GtkXlfdSelection *fontsel;
  gint row;
...
static void
gtk_xlfd_selection_select_font (GtkWidget      *w,
				gint	        row,
				gint	        column,
				GdkEventButton *bevent,
				gpointer        data)
...
static gint
gtk_xlfd_selection_on_clist_key_press (GtkWidget        *clist,
				       GdkEventKey      *event,
				       GtkXlfdSelection *fontsel)
{
#ifdef FONTSEL_DEBUG
...
static gboolean
gtk_xlfd_selection_select_next (GtkXlfdSelection *fontsel,
				GtkWidget        *clist,
				gint		  step)
{
  GList *selection;
...
static void
gtk_xlfd_selection_show_available_styles (GtkXlfdSelection *fontsel)
{
  FontInfo *font;
  FontStyle *styles;
  ...
  for (style = 0; style < font->nstyles; style++)
    {
      if (styles[style].flags & GTK_XLFD_DISPLAYED)
	continue;
...
static void
gtk_xlfd_selection_select_best_style(GtkXlfdSelection *fontsel,
				     gboolean	       use_first)
{
  FontInfo *font;
  FontStyle *styles;
...
static void
gtk_xlfd_selection_select_style (GtkWidget      *w,
				 gint	        row,
				 gint	        column,
				 GdkEventButton *bevent,
				 gpointer        data)
...
static void
gtk_xlfd_selection_show_available_sizes (GtkXlfdSelection *fontsel)
{
  FontInfo *font;
  ...
  if (!((style->flags & GTK_XLFD_SCALABLE_BITMAP
	 && type_filter & GTK_XLFD_SCALABLE_BITMAP)
	|| (style->flags & GTK_XLFD_SCALABLE
	    && type_filter & GTK_XLFD_SCALABLE)))
    nstandard_sizes = 0;
...
static void
gtk_xlfd_selection_update_size (GtkXlfdSelection *fontsel)
{
  gint new_size;
...
/* If the user hits return in the font size entry, we change to the new font
   size. */
static void
gtk_xlfd_selection_size_activate (GtkWidget *w,
				  gpointer   data)
{
...
/* This tries to select the closest size to the current size, though it
   ...
   Note: this will load a font. */
static void
gtk_xlfd_selection_select_best_size(GtkXlfdSelection *fontsel)
{
  FontInfo *font;
  FontStyle *styles, *style;
  gchar *text;
...
static void
gtk_xlfd_selection_select_size (GtkWidget      *w,
				gint	        row,
				gint	        column,
				GdkEventButton *bevent,
				gpointer        data)
...
static void
gtk_xlfd_selection_metric_callback (GtkWidget *w,
				    gpointer   data)
{
...
static guint16
gtk_xlfd_selection_field_to_index (gchar **table,
				   gint    ntable,
				   gchar  *field)
{
  gint i;
...
static gboolean
gtk_xlfd_selection_load_font (GtkXlfdSelection *fontsel)
{
  GdkFont *font;
...
static void
gtk_xlfd_selection_update_preview (GtkXlfdSelection *fontsel)
{
...
static void
gtk_xlfd_selection_switch_page (GtkWidget       *w,
				GtkNotebookPage *page,
				gint             page_num,
				gpointer         data)
{
...
static void
gtk_xlfd_selection_show_font_info (GtkXlfdSelection *fontsel)
{
  Atom font_atom, atom;
  Bool status;
  char *name;
  ...
  if (fontsel->font)
      ...
      if (status == True)
	{
	  ...
	  for (i = 0; i < GTK_XLFD_NUM_FIELDS; i++)
	    {
	      field = gtk_xlfd_selection_get_xlfd_field (name, i,
							 field_buffer);
	      if (i == XLFD_SLANT)
		field = gtk_xlfd_selection_expand_slant_code(field);
	      else if (i == XLFD_SPACING)
...
static gchar*
gtk_xlfd_selection_expand_slant_code(gchar *slant)
{
...
static gchar*
gtk_xlfd_selection_expand_spacing_code(gchar *spacing)
{
...
static void
gtk_xlfd_selection_select_filter	     (GtkWidget      *w,
					      gint	      row,
					      gint	      column,
					      GdkEventButton *bevent,
					      GtkXlfdSelection *fontsel)
{
  ...
  if (row == 0)
	...
	if (fontsel->filter_clists[prop] == w)
      ...
      if (gtk_xlfd_selection_filter_state (fontsel, GTK_XLFD_FILTER_BASE,
...
/* This is called when the main notebook page is selected. It checks if the
   ...
   turned off. */
static void
gtk_xlfd_selection_update_filter     (GtkXlfdSelection *fontsel)
{
  GtkWidget *clist;
  GList *selection;
  ...
  new_font_type |= (GTK_TOGGLE_BUTTON(fontsel->type_scalable_button)->active
    ? GTK_XLFD_SCALABLE : 0);
  new_font_type |= (GTK_TOGGLE_BUTTON(fontsel->type_scaled_bitmaps_button)->active ? GTK_XLFD_SCALABLE_BITMAP : 0);
  new_font_type &= base_font_type;
  new_font_type |= (~base_font_type & user_font_type);
  if (new_font_type != (GTK_XLFD_BITMAP | GTK_XLFD_SCALABLE))
...
static void
gtk_xlfd_selection_show_available_fonts     (GtkXlfdSelection *fontsel)
{
...
static gboolean
gtk_xlfd_selection_style_visible(GtkXlfdSelection *fontsel,
				 FontInfo         *font,
				 gint              style_index)
{
  FontStyle *styles, *style;
...
static void
gtk_xlfd_selection_reset_filter	     (GtkWidget      *w,
				      GtkXlfdSelection *fontsel)
{
...
static void
gtk_xlfd_selection_on_clear_filter     (GtkWidget      *w,
					GtkXlfdSelection *fontsel)
{
...
static void
gtk_xlfd_selection_clear_filter     (GtkXlfdSelection *fontsel)
{
  GtkXlfdFilter *filter;
  gint prop;
...
void
gtk_xlfd_selection_set_filter	(GtkXlfdSelection *fontsel,
				 GtkXlfdFilterType filter_type,
				 ...
				 gchar		 **charsets)
{
  ...
  if (filter_type == GTK_XLFD_FILTER_BASE)
      ...
      if (font_type & GTK_XLFD_BITMAP)
	{
	  ...
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_bitmaps_button), user_font_type & GTK_XLFD_BITMAP);
	}
      else
	{
      ...
      if (font_type & GTK_XLFD_SCALABLE)
	{
	  ...
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scalable_button), user_font_type & GTK_XLFD_SCALABLE);
	}
      else
	{
      ...
      if (font_type & GTK_XLFD_SCALABLE_BITMAP)
	{
	  ...
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scaled_bitmaps_button), user_font_type & GTK_XLFD_SCALABLE_BITMAP);
	}
      else
	{
  ...
  else
    {
      base_font_type = fontsel->filters[GTK_XLFD_FILTER_BASE].font_type;
      if (base_font_type & GTK_XLFD_BITMAP)
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_bitmaps_button), font_type & GTK_XLFD_BITMAP);

      if (base_font_type & GTK_XLFD_SCALABLE)
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scalable_button), font_type & GTK_XLFD_SCALABLE);

      if (base_font_type & GTK_XLFD_SCALABLE_BITMAP)
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scaled_bitmaps_button), font_type & GTK_XLFD_SCALABLE_BITMAP);

      /* If the user filter is not the default, make the 'Reset Filter' button
	 sensitive. */
      filter_set = FALSE;
      if (font_type != (GTK_XLFD_BITMAP | GTK_XLFD_SCALABLE))
	filter_set = TRUE;
      for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++)
	{
	  if (filter->property_nfilters[prop] != 0)
...
static void
gtk_xlfd_selection_update_filter_lists (GtkXlfdSelection *fontsel)
{
  GtkWidget *clist;
  ...
  for (prop = 0; prop < GTK_NUM_FONT_PROPERTIES; prop++)
      ...
      for (row = 1; row < GTK_CLIST(clist)->rows; row++)
	{
	  ...
	  if (gtk_xlfd_selection_filter_state (fontsel, GTK_XLFD_FILTER_USER,
					       prop, index) == FILTERED
	      && fg == NULL)
	    gtk_clist_select_row (GTK_CLIST (clist), row, 0);
	  else
...
static GtkXlfdPropertyFilterState
gtk_xlfd_selection_filter_state (GtkXlfdSelection *fontsel,
				 GtkXlfdFilterType filter_type,
				 gint		  property,
				 gint		  index)
{
 ...
 *****************************************************************************/
static void
gtk_xlfd_selection_get_fonts (void)
{
  gchar **xfontnames;
  GSList **fontnames;
  gchar *fontname;
  ...
  for (i = 0; i < fontsel_info->nfonts; i++)
      ...
      while (temp_list)
	{
	  ...
	  for (prop = 0; prop < GTK_NUM_STYLE_PROPERTIES; prop++)
		...
		= gtk_xlfd_selection_insert_field (fontname, prop);
	    }
	  ...
	  field = gtk_xlfd_selection_get_xlfd_field (fontname, XLFD_PIXELS,
						     field_buffer);
	  pixels = atoi(field);
	  ...
	  field = gtk_xlfd_selection_get_xlfd_field (fontname,
						     XLFD_RESOLUTION_X,
						     field_buffer);
	  res_x = atoi(field);
  ...
  for (i = 0; i < fontsel_info->nfonts; i++)
      ...
      for (style = 0; style < font->nstyles; style++)
	{
	  ...
	  if (styles->flags & GTK_XLFD_BITMAP)
	    g_message("Bitmapped font  ");
	  if (styles->flags & GTK_XLFD_SCALABLE)
	    g_message("Scalable font  ");
	  if (styles->flags & GTK_XLFD_SCALABLE_BITMAP)
...
static void
gtk_xlfd_selection_insert_font (GSList		      *fontnames[],
				gint		      *ntable,
				gchar		      *fontname)
{
  FontInfo *table;
  ...
  family = gtk_xlfd_selection_get_xlfd_field (fontname, XLFD_FAMILY,
					      family_buffer);
  if (!family)
    return;
...
static gint
gtk_xlfd_selection_insert_field (gchar		       *fontname,
				 gint			prop)
{
  gchar field_buffer[XLFD_MAX_FIELD_LEN];
  gchar *field;
  ...
  field = gtk_xlfd_selection_get_xlfd_field (fontname, xlfd_index[prop],
					     field_buffer);
  if (!field)
    return 0;
...
GdkFont*
gtk_xlfd_selection_get_font (GtkXlfdSelection *fontsel)
{
...
gchar *
gtk_xlfd_selection_get_font_name (GtkXlfdSelection *fontsel)
{
  FontInfo *font;
  ...
  return gtk_xlfd_selection_create_xlfd (fontsel->size,
					 fontsel->metric,
					 foundry_str,
					 family_str,
					 property_str[WEIGHT],
...
gboolean
gtk_xlfd_selection_set_font_name (GtkXlfdSelection *fontsel,
				  const gchar      *fontname)
{
  ...
  if (!gtk_xlfd_selection_is_xlfd_font_name(fontname))
    return FALSE;
  ...
  field = gtk_xlfd_selection_get_xlfd_field (fontname, XLFD_FOUNDRY,
					     field_buffer);
...
static gint
gtk_xlfd_selection_find_font (GtkXlfdSelection *fontsel,
			      gchar	       *family,
			      guint16		foundry)
{
  FontInfo *font_info;
...
void
gtk_xlfd_selection_set_preview_text  (GtkXlfdSelection *fontsel,
				      const gchar	  *text)
{
 ...
 */
static gboolean
gtk_xlfd_selection_is_xlfd_font_name (const gchar *fontname)
{
  gint i = 0;
  gint field_len = 0;
 ...
 */
static gchar*
gtk_xlfd_selection_get_xlfd_field (const gchar *fontname,
				   FontField    field_num,
				   gchar       *buffer)
{
  const gchar *t1, *t2;
 ...
 */
static gchar *
gtk_xlfd_selection_create_xlfd (gint		  size,
				GtkXlfdMetricType metric,
				gchar		  *foundry,
				gchar		  *family,
				gchar		  *weight,
...
guint
gtk_xlfd_selection_dialog_get_type (void)
{
  ...
  if (!font_selection_dialog_type)
    {
      ...
      {
	"GtkXlfdSelectionDialog",
	sizeof (GtkXlfdSelectionDialog),
	sizeof (GtkXlfdSelectionDialogClass),
	(GtkClassInitFunc) gtk_xlfd_selection_dialog_class_init,
...
static void
gtk_xlfd_selection_dialog_class_init (GtkXlfdSelectionDialogClass *klass)
{
  GtkObjectClass *object_class;
...
static void
gtk_xlfd_selection_dialog_init (GtkXlfdSelectionDialog *fontseldiag)
{
  ...
  gtk_signal_connect (GTK_OBJECT (fontseldiag), "configure_event",
		      (GtkSignalFunc) gtk_xlfd_selection_dialog_on_configure,
		      fontseldiag);
...
GtkWidget*
gtk_xlfd_selection_dialog_new	(const gchar	  *title)
{
...
gchar*
gtk_xlfd_selection_dialog_get_font_name	(GtkXlfdSelectionDialog *fsd)
{
  return gtk_xlfd_selection_get_font_name(GTK_XLFD_SELECTION(fsd->fontsel));
}

GdkFont*
gtk_xlfd_selection_dialog_get_font	(GtkXlfdSelectionDialog *fsd)
{
  return gtk_xlfd_selection_get_font(GTK_XLFD_SELECTION(fsd->fontsel));
}

gboolean
gtk_xlfd_selection_dialog_set_font_name	(GtkXlfdSelectionDialog *fsd,
					 const gchar	  *fontname)
{
  return gtk_xlfd_selection_set_font_name(GTK_XLFD_SELECTION(fsd->fontsel),
					  fontname);
}

void
gtk_xlfd_selection_dialog_set_filter	(GtkXlfdSelectionDialog *fsd,
					 GtkXlfdFilterType filter_type,
					 GtkXlfdType	   font_type,
					 gchar		 **foundries,
					 gchar		 **weights,
					 ...
					 gchar		 **charsets)
{
  gtk_xlfd_selection_set_filter (GTK_XLFD_SELECTION (fsd->fontsel),
...
void
gtk_xlfd_selection_dialog_set_preview_text (GtkXlfdSelectionDialog *fsd,
					    const gchar	  *text)
{
...
/* This turns auto-shrink off if the user resizes the width of the dialog.
   ...
   width. */
static gint
gtk_xlfd_selection_dialog_on_configure (GtkWidget         *widget,
					GdkEventConfigure *event,
					GtkXlfdSelectionDialog *fsd)
{
