...
GtkWidget* mc_geometry_config_widget_new(void) {
  ...
  for (count = 0; count < MC_GEOMETRIES; count++) {
    ...
    g_signal_connect(entry, "changed", G_CALLBACK(geometry_selected), &new_values[count]);
  }
...
void mc_update_geometry(void) {
  ...
  if (is_changed) {
    ...
    sprintf(value, "%sx%s", new_values[MC_GEOMETRY_COLUMNS], new_values[MC_GEOMETRY_ROWS]);
